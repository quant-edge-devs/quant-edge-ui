...
const StatCard = ({ label, value }: { label: string; value: React.ReactNode }) => (
  <div className="rounded-2xl border border-black/[0.07] dark:border-white/[0.07] bg-black/[0.04] dark:bg-white/[0.03] p-4 backdrop-blur-sm">
    <div className="mb-1 text-[11px] font-semibold uppercase tracking-widest text-gray-400 dark:text-white/35">{label}</div>
    ...
    <div className="min-h-screen w-full overflow-x-hidden bg-[#f5f3ff] dark:bg-[#06050f] text-gray-900 dark:text-white">
      ...
      <main className="relative z-10 mx-auto max-w-4xl px-6 py-10">
        <div className="mb-8">
          ...
          <p className="mt-1 text-sm text-gray-400 dark:text-white/40">Search for real-time company information</p>
        </div>

        <div className="mb-8 flex items-center gap-3">
          <input
            className="flex-1 rounded-2xl border border-black/[0.07] dark:border-white/[0.07] bg-black/[0.03] dark:bg-white/[0.03] px-5 py-3 text-sm placeholder-gray-400 dark:placeholder-white/25 backdrop-blur-sm outline-none transition focus:border-purple-500/40 focus:bg-black/[0.05] dark:focus:bg-white/[0.05]"
          ...
          />
          <button
            className="cursor-pointer rounded-2xl border border-purple-500/30 bg-purple-500/10 px-6 py-3 text-sm font-semibold text-purple-600 dark:text-purple-300 backdrop-blur-sm transition hover:border-purple-500/50 hover:bg-purple-500/20 hover:text-purple-700 dark:hover:text-white"
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center gap-4 py-24 text-gray-400 dark:text-white/40">
            <div className="h-8 w-8 animate-spin rounded-full border-2 border-black/10 dark:border-white/10 border-t-purple-500" />
          ...
          <div className="flex flex-col gap-5">
              ...
              <div className="flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
                <div className="flex items-center gap-4">
                  {imageError || !data.image ? (
                    <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-black/[0.08] dark:bg-white/[0.08] text-sm font-bold text-gray-500 dark:text-white/60">
                  ...
                  <div>
                    <div className="flex items-center gap-2.5">
                      ...
                      <span className="text-base text-gray-500 dark:text-white/55">{data.companyName}</span>
                    </div>
                    <div className="mt-1.5 flex flex-wrap gap-1.5">
                      <span className="rounded-full border border-purple-500/25 bg-purple-500/10 px-2.5 py-0.5 text-[11px] font-semibold text-purple-600 dark:text-purple-300">{data.exchange}</span>
                      <span className="rounded-full border border-violet-500/25 bg-violet-500/10 px-2.5 py-0.5 text-[11px] font-semibold text-violet-600 dark:text-violet-300">{data.sector}</span>
                      <span className="rounded-full border border-fuchsia-500/20 bg-fuchsia-500/10 px-2.5 py-0.5 text-[11px] font-semibold text-fuchsia-600 dark:text-fuchsia-300">{data.industry}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="text-right">
                  ...
                  <div className={`mt-1 text-sm font-semibold ${isPositive ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-500 dark:text-red-400'}`}>
                    {isPositive ? '▲' : '▼'} {Math.abs(data.change).toFixed(2)} ({data.changePercentage.toFixed(2)}%)
                  </div>
                  <div className="mt-0.5 text-[11px] text-gray-300 dark:text-white/30">{data.currency}</div>
                </div>
              </div>
            </div>
            ...
            {data.description && (
              <div className="rounded-2xl border border-black/[0.07] dark:border-white/[0.07] bg-black/[0.04] dark:bg-white/[0.03] p-6 backdrop-blur-sm">
                <div className="mb-2 text-[11px] font-semibold uppercase tracking-widest text-gray-400 dark:text-white/35">About</div>
                <p className="text-sm leading-relaxed text-gray-500 dark:text-white/60">{data.description}</p>
              </div>
            )}

            <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
              <div className="flex items-center gap-3 rounded-2xl border border-black/[0.07] dark:border-white/[0.07] bg-black/[0.04] dark:bg-white/[0.03] p-4 backdrop-blur-sm">
                ...
                <div>
                  <div className="text-[11px] text-gray-400 dark:text-white/35">CEO</div>
                  <div className="text-sm font-semibold">{data.ceo || '-'}</div>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3 rounded-2xl border border-black/[0.07] dark:border-white/[0.07] bg-black/[0.04] dark:bg-white/[0.03] p-4 backdrop-blur-sm">
                ...
                <div className="min-w-0">
                  <div className="text-[11px] text-gray-400 dark:text-white/35">Website</div>
                  {data.website ? (
                    <a
                      href={data.website}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="truncate text-sm font-semibold text-purple-500 dark:text-purple-400 hover:text-purple-400 dark:hover:text-purple-300"
                    >
              ...
              <div className="flex items-center gap-3 rounded-2xl border border-black/[0.07] dark:border-white/[0.07] bg-black/[0.04] dark:bg-white/[0.03] p-4 backdrop-blur-sm">
                ...
                <div>
                  <div className="text-[11px] text-gray-400 dark:text-white/35">Headquarters</div>
