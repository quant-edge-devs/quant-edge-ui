...
export const ContactUs = () => {
  ...
  return (
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[#f5f3ff] dark:bg-[#06050f] px-4 py-12 text-gray-900 dark:text-white">
      ...
      <div className="relative z-10 w-full max-w-md animate-fade-up">
        ...
        <div className="rounded-2xl border border-black/[0.07] dark:border-white/[0.07] bg-black/[0.04] dark:bg-white/[0.03] p-8 backdrop-blur-sm">
          ...
          <p className="mb-7 text-sm text-gray-500 dark:text-white/45">
            Reach us directly at{' '}
            <a href={`mailto:${contactEmail}`} className="text-purple-500 dark:text-purple-400 hover:text-purple-400 dark:hover:text-purple-300">
              {contactEmail}
            </a>
          </p>
            ...
            {({ isSubmitting }) => (
              <Form className="flex flex-col gap-4">
                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-medium text-gray-500 dark:text-white/60" htmlFor="name">Name</label>
                  <Field id="name" name="name" type="text" placeholder="Your name" className={fieldCls} />
                  <ErrorMessage name="name" component="div" className="text-[11px] text-red-500 dark:text-red-400" />
                </div>
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <ErrorMessage name="email" component="div" className="text-[11px] text-red-500 dark:text-red-400" />
                </div>

                <div className="flex flex-col gap-1.5">
                  <label className="text-xs font-medium text-gray-500 dark:text-white/60" htmlFor="message">Message</label>
                  <Field
                    as="textarea"
                    id="message"
                    name="message"
                  ...
                  />
                  <ErrorMessage name="message" component="div" className="text-[11px] text-red-500 dark:text-red-400" />
                </div>
                ...
                {sent && (
                  <div className="rounded-lg border border-emerald-500/20 bg-emerald-500/10 px-3 py-2 text-center text-xs text-emerald-600 dark:text-emerald-400">
                    Message sent — we'll get back to you shortly.
                  </div>
                )}
                {error && (
                  <div className="rounded-lg border border-red-500/20 bg-red-500/10 px-3 py-2 text-center text-xs text-red-500 dark:text-red-400">
                    {error}
                  </div>
                )}
              </Form>
